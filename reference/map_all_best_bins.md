...
- blase_data:
  ...
  holding the bins and pseudobulks.

- bulk_data:

  Dataframe. The whole bulk read matrix as a dataframe. Each row should
...
- confidence_level:
  ...
  Decimal between 0-1. The confidence interval to calculate for
  mappings. Defaults to 0.9, or 90%.

- BPPARAM:

  The
  [BiocParallel::BiocParallelParam](https://rdrr.io/pkg/BiocParallel/man/BiocParallelParam-class.html)
  for multithreading if desired. Defaults to
  [`BiocParallel::SerialParam()`](https://rdrr.io/pkg/BiocParallel/man/SerialParam-class.html)

- metric:

  Character. The metric to use to compare mappings. One of: 'spearman',
  'pearson', 'kendall', 'cosine_similarity', 'euclidean', 'manhattan.'
