...
res <- SCDC(processed_MCA_PF_SCE, 'pseudotime_bin', bulk, 'DAY')
...
resm <- MUSIC(processed_MCA_PF_SCE, 'pseudotime_bin', bulk, 'DAY')
...
cibersortx_results_plot = ggplot(to_plot, aes(variable, Mixture)) +
  ...
  scale_x_discrete(limits = paste0('c', as.character(sort(unique(processed_MCA_PF_SCE$pseudotime_bin))))) + 
  ggtitle("CibersortX")
...
```{r, include=FALSE}
# TODO get calls outside of here so we can also get calls from MUSIC, SCDC and CibersortX
