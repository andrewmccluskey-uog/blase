...
article_dir = fs::path(root_dir, "3d7_lifecycle")
if (!dir.exists(article_dir)) {
  dir.create(article_dir, recursive = TRUE)
}

bulk_path = fs::path(article_dir, "3d7_bulk_timepoints", ext="xlsx")
...
```{r}
waves = get_waves(processed_MCA_PF_SCE, "slingPseudotime_1", BPPARAM = BiocParallel::MulticoreParam(N_CORES))
...
```{r}
gene_selection_matrix(processed_MCA_PF_SCE, waves, BPPARAM = BiocParallel::MulticoreParam(N_CORES), target_matrix_size=nrow(waves))
```

#### TradeSeq Genes

```{r}
gene_selection_matrix(processed_MCA_PF_SCE, waves, genes=genelist[0:300], BPPARAM = BiocParallel::MulticoreParam(N_CORES), target_matrix_size=300)
```

#### Bozdech Genes

```{r}
gene_selection_matrix(processed_MCA_PF_SCE, waves, genes=rownames(fourier_power_genes), BPPARAM = BiocParallel::MulticoreParam(N_CORES), target_matrix_size=length(unique(rownames(fourier_power_genes))))
```
...
```{r}
gridExtra::grid.arrange(
  blase_tradeseq_results_plot + scale_y_discrete(limits = paste(0:47, "hpi")) + ggtitle("BLASE (TradeSeq)"),
  blase_bozdech_results_plot + scale_y_discrete(limits = paste(0:47, "hpi")) + ggtitle("BLASE (Bozdech et al.)"),
