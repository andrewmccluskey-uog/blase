...
article_dir <- fs::path(root_dir, "gene_selection_comparison")
if (!dir.exists(article_dir)) {
    dir.create(article_dir, recursive = TRUE)
}

bulk_path <- fs::path(article_dir, "otto_pb_bulk_timepoints", ext = "xlsx")
...
genes(x) <- genelist[1:GENES_TO_USE_COUNT]

genes_tradeseq <- genes(x)
...
genes_markergenes_by_bin <- genes(x)
...
```{r}
waves <- get_waves(processed_MCA_PB_SCE, BPPARAM = BiocParallel::MulticoreParam(N_CORES))
...
```{r}
gene_selection_matrix(processed_MCA_PB_SCE, waves, target_matrix_size = 1000, BPPARAM = BiocParallel::MulticoreParam(N_CORES))
```
...
x <- as.BlaseData(processed_MCA_PB_SCE, pseudotime_slot = "slingPseudotime_1", n_bins = BUCKETS_COUNT, split_by = "pseudotime_range")
genes(x) <- top_genes_fourier_top_r2
...
x <- as.BlaseData(processed_MCA_PB_SCE, pseudotime_slot = "slingPseudotime_1", n_bins = BUCKETS_COUNT, split_by = "pseudotime_range")
genes(x) <- top_genes_fourier_top_r2_spread
...
x <- as.BlaseData(processed_MCA_PB_SCE, pseudotime_slot = "slingPseudotime_1", n_bins = BUCKETS_COUNT, split_by = "pseudotime_range")
genes(x) <- top_genes_fourier_top_amp
...
x <- as.BlaseData(processed_MCA_PB_SCE, pseudotime_slot = "slingPseudotime_1", n_bins = BUCKETS_COUNT, split_by = "pseudotime_range")
genes(x) <- top_genes_fourier_top_amp_spread
...
x <- as.BlaseData(processed_MCA_PB_SCE, pseudotime_slot = "slingPseudotime_1", n_bins = BUCKETS_COUNT, split_by = "pseudotime_range")
genes(x) <- top_genes_fourier_top_power
...
x <- as.BlaseData(processed_MCA_PB_SCE, pseudotime_slot = "slingPseudotime_1", n_bins = BUCKETS_COUNT, split_by = "pseudotime_range")
genes(x) <- top_genes_fourier_top_power_spread
...
reduced_sim_mat <- redim_matrix(sim_mat, target_height = 100, target_width = 100, BPPARAM = BiocParallel::MulticoreParam(N_CORES))
...
get_earthmovers_distance_from_diagonal <- function(expression_matrix, title) {
    ...
    reduced_mat <- redim_matrix(expression_matrix, target_height = 40, target_width = 40, BPPARAM = BiocParallel::MulticoreParam(N_CORES))
    reduced_sim_mat <- redim_matrix(sim_mat, target_height = 40, target_width = 40, BPPARAM = BiocParallel::MulticoreParam(N_CORES))
...
```{r}
top_tradeseq_phases_plot <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = genes_tradeseq, target_matrix_size = length(genes_tradeseq), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("TradeSeq")

top_marker_phases_plot <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = genes_markergenes, target_matrix_size = length(genes_markergenes), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("Top Cluster Markers")

length(genes_markergenes_by_bin)
genes_markergenes_by_bin

top_bin_marker_phases_plot <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = genes_markergenes_by_bin, target_matrix_size = length(unique(genes_markergenes_by_bin)), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("Top Bin Markers")

fourier_top_r2_phases <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = top_genes_fourier_top_r2, target_matrix_size = length(top_genes_fourier_top_r2), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("Top Fourier R2")

fourier_top_r2_spread_phases <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = top_genes_fourier_top_r2_spread, target_matrix_size = length(top_genes_fourier_top_r2_spread), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("Top Fourier R2 Spread")

fourier_top_amp_phases <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = top_genes_fourier_top_amp, target_matrix_size = length(top_genes_fourier_top_amp), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("Top Fourier Amplitude")

fourier_top_amp_spread_phases <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = top_genes_fourier_top_amp_spread, target_matrix_size = length(top_genes_fourier_top_amp_spread), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("Top Fourier Amplitude Spread")

fourier_top_power_phases <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = top_genes_fourier_top_power, target_matrix_size = length(top_genes_fourier_top_power), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("Top Fourier Power")

fourier_top_power_spread_phases <- gene_selection_matrix(processed_MCA_PB_SCE, waves, genes = top_genes_fourier_top_power_spread, target_matrix_size = length(top_genes_fourier_top_power_spread), BPPARAM = BiocParallel::MulticoreParam(N_CORES)) + ggplot2::ggtitle("Top Fourier Power Spread")
