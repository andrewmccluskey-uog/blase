...
gene_selection_matrix(processed_MCA_PF_SCE, waves, genes=genes(blase_data), target_matrix_size=length(unique(genes(blase_data))))
```
...
gridExtra::grid.arrange(
  blase::plot_mapping_result_heatmap(rev(mapping_results[1:2]), annotate = TRUE) + ggtitle("WT Normal Conditions"),
  blase::plot_mapping_result_heatmap(rev(mapping_results[3:4]), annotate = TRUE) + ggtitle("WT Heat Shock"),
  ncol=1
)
gridExtra::grid.arrange(
  blase::plot_mapping_result_heatmap(rev(mapping_results[9:10]), annotate = TRUE) + ggtitle("PB31 Normal Conditions"),
  blase::plot_mapping_result_heatmap(rev(mapping_results[11:12]), annotate = TRUE) + ggtitle("PB31 Heat Shock"),
  ncol=1
)
```
...
any(genes(blase_data) == "PF3D7_1432400")
```
...
### SCDC
```{r SCDC}
scdc_results = SCDC(processed_MCA_PF_SCE, 'pseudotime_bin', bulk)
...
scdc_results_plot = ggplot(to_plot, aes(variable, BulkID)) +
  ...
  scale_x_discrete(limits = seq_len(8)) +
  ggtitle("SCDC")
...
cibersortx_results_plot = ggplot(to_plot, aes(variable, Mixture)) +
  geom_tile(aes(fill = value), colour = "white") +
  scale_fill_gradient(low = "white", high = "red", limits=c(0,1)) +
  scale_y_discrete(limits = gsub(x = sample_order, "_", "")) +
  scale_x_discrete(limits = seq_len(8)) + 
  ggtitle("CibersortX")
...
### Final Comparison
```{r}
blase::plot_mapping_result_heatmap(mapping_results, annotate = TRUE) + ggtitle('BLASE')
cibersort_results_plot + ggtitle('CibersortX')
scdc_results_plot + ggtitle('ScDC')
```
