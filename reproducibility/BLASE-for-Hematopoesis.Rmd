...
genes(blase_data) = genelist[1:160]
...
# See also `table (blase_train$pseudotime_bin, blase_train$Sample)`
res <- SCDC(blase_train, 'pseudotime_bin', bulks_df, 's')
...
scdc_results_plot = ggplot(to_plot, aes(variable, BulkID)) +
  ...
  scale_x_discrete(limits = seq_len(7)) +
  ggtitle("SCDC")
scdc_results_plot
```
...
resm <- MUSIC(blase_train, 'pseudotime_bin', bulks_df, 's')
...
```{r cibersort}
cibersort <- CibersortX(blase_train, 'pseudotime_bin', bulks_df, N_CORES)
...
cibersortx_results_plot <- ggplot(to_plot, aes(variable, Mixture)) +
  ...
  scale_fill_gradient(low = "white", high = "red", limits=c(0,1)) +
  scale_y_discrete(limits = expected_trajectory) +
  scale_x_discrete(limits = paste0('c', seq_len(7))) + 
  ggtitle("CibersortX")
...
```{r}
blase_data = as.BlaseData(blase_train, n_bins = 7)
genes(blase_data) = genelist[1:400]
