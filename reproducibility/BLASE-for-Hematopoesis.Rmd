...
## TODO: WHY ARE SOME CELL TYPES MISSING?
```
...
ggplot(to_plot, aes(variable, BulkID)) +
  ...
  geom_text(aes(label = round(value, 2))) +
  scale_fill_gradient(low = "white", high = "red", limits=c(0,1))
## TODO REORDER THIS PLOT TO MATCH EXPECTED ORDER - SHOULD BE CODE FOR THIS IN MCA/ZHANG FILE
...
### Fewer Bins
```{r}
blase_data = as.BlaseData(processed_weng_2024_haematopoiesis, n_bins = 3)
blase_data@genes = genelist[1:160]
low_bins_results = c()
for (type in expected_trajectory) {
...
### More Bins
```{r}
blase_data = as.BlaseData(processed_weng_2024_haematopoiesis, n_bins = 20)
blase_data@genes = genelist[1:160]
high_bins_results = c()
for (type in expected_trajectory) {
