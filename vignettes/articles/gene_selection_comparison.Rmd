...
get_by_spread = function(waves, n_groups, parameter, top_n_per_group=1) {
  ...
  for(i in seq(from=1, to=max(waves$phase), length.out=n_groups)) {
    stepsize = max(waves$phase)/n_groups
    waves_in_block = waves[waves$phase<i+stepsize,]
...
```{r}
best_amp_waves_spread = best_r2_waves_spread = get_by_spread(waves, GENES_TO_USE_COUNT, "amp")
...
best_power_waves_spread = best_r2_waves_spread = get_by_spread(waves, GENES_TO_USE_COUNT, "power")
