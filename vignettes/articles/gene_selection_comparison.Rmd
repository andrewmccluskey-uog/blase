...
```{r}
#load_all('code/PhD/R/atgnat')
best_r2_waves_spread = data.frame()
for(i in seq(from=1, to=max(waves$phase), length.out=GENES_TO_USE_COUNT)) {
  stepsize = max(waves$phase)/GENES_TO_USE_COUNT
...
best_amp_waves_spread = data.frame()
for(i in seq(from=1, to=max(waves$phase), length.out=GENES_TO_USE_COUNT)) {
  stepsize = max(waves$phase)/GENES_TO_USE_COUNT
  waves_in_block = waves[waves$phase>i-(stepsize/2) & waves$phase<i+(stepsize/2),]
  best_wave_in_block = waves_in_block[order(-waves_in_block$amp),][1,]
  best_amp_waves_spread = rbind(best_amp_waves_spread, best_wave_in_block)
}
...
```
### Amplitude Power Spread

``` {r}

best_power_waves_spread = data.frame()
for(i in seq(from=1, to=max(waves$phase), length.out=GENES_TO_USE_COUNT)) {
  stepsize = max(waves$phase)/GENES_TO_USE_COUNT
  waves_in_block = waves[waves$phase>i-(stepsize/2) & waves$phase<i+(stepsize/2),]
  best_wave_in_block = waves_in_block[order(-waves_in_block$power),][1,]
  best_power_waves_spread = rbind(best_power_waves_spread, best_wave_in_block)
}
...
## Fourier Map Correlations
```{r}
heatmap_counts=normcounts(sce)[order(waves$phase),order(sce$slingPseudotime_1)]
mat = heatmap_counts[rownames(heatmap_counts) %in% top_genes_fourier_top_power_spread,]
...
sim_mat = matrix(0, ncol=nrow(mat),nrow=nrow(mat))
...
sim_mat <- sim_mat/sum(sim_mat) * sum(mat)

#blah = redim_matrix(sim_mat, target_height = nrow(sim_mat), target_width = 1000, n_core=2)
blah <- reshape2::melt(sim_mat, c("gene", "cell"), value.name = "expr")
ggplot2::ggplot(data=blah, ggplot2::aes(x=cell,y=gene,fill=expr)) +
  ggplot2::geom_tile() + 
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank()) +
  ggplot2::scale_fill_gradient(low = "white",
                      high = "red",
                      guide = "colorbar",
                      limits=c(0, max(blah$expr))) + 
  ggplot2::ggtitle("Test SIM_MAT")
rm(blah)

blah = redim_matrix(mat, target_height = nrow(sim_mat), target_width = nrow(sim_mat), n_core=2)
blah <- reshape2::melt(blah, c("gene", "cell"), value.name = "expr")
ggplot2::ggplot(data=blah, ggplot2::aes(x=cell,y=gene,fill=expr)) +
  ggplot2::geom_tile() + 
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank()) +
  ggplot2::scale_fill_gradient(low = "white",
                      ...
                      guide = "colorbar",
                      limits=c(0, max(blah$expr))) + 
  ggplot2::ggtitle("Test MAT")
rm(blah)

#library(dad)
#wasserstein(mat, sim_mat)

dim(mat)
dim(sim_mat)

reduced_mat = redim_matrix(mat, target_height = nrow(mat), target_width = nrow(mat), n_core=2)
reduced_sim_mat = redim_matrix(sim_mat, target_height = nrow(sim_mat), target_width = nrow(mat), n_core=2)
...
small_heatmap_counts_ordered = redim_matrix(heatmap_counts[order(waves$phase),], target_height = 1000, target_width = 1000, n_core=2)
heatmap_counts_ordered.df <- reshape2::melt(small_heatmap_counts_ordered, c("gene", "cell"), value.name = "log_expression")


top_r2_phases = heatmap_counts[rownames(heatmap_counts) %in% top_genes_fourier_top_r2,]
top_r2_phases = redim_matrix(top_r2_phases, target_height = nrow(top_r2_phases), target_width = 1000, n_core=2)
top_r2_phases <- reshape2::melt(top_r2_phases, c("gene", "cell"), value.name = "log_expression")
fourier_top_r2_phases = ggplot2::ggplot(data=top_r2_phases,ggplot2::aes(x=cell,y=gene,fill=log_expression)) +
  ggplot2::geom_tile() + 
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank()) +
  ggplot2::scale_fill_gradient(low = "white",
                      high = "red",
  ...
  ggplot2::ggtitle("Top Fourier R2 Genes")

top_amp_phases = heatmap_counts[rownames(heatmap_counts) %in% top_genes_fourier_top_amp,]
top_amp_phases = redim_matrix(top_amp_phases, target_height = nrow(top_amp_phases), target_width = 1000, n_core=2)
top_amp_phases <- reshape2::melt(top_amp_phases, c("gene", "cell"), value.name = "log_expression")
fourier_top_amp_phases = ggplot2::ggplot(data=top_amp_phases,ggplot2::aes(x=cell,y=gene,fill=log_expression)) +
  ggplot2::geom_tile() + 
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank()) +
  ggplot2::scale_fill_gradient(low = "white",
                      high = "red",
                      guide = "colorbar",
                      limits=c(0, max(heatmap_counts)))+ 
  ggplot2::ggtitle("Top Fourier Amp Genes")

top_power_phases = heatmap_counts[rownames(heatmap_counts) %in% top_genes_fourier_top_power,]
top_power_phases = redim_matrix(top_power_phases, target_height = nrow(top_power_phases), target_width = 1000, n_core=2)
top_power_phases <- reshape2::melt(top_power_phases, c("gene", "cell"), value.name = "log_expression")
fourier_top_power_phases = ggplot2::ggplot(data=top_power_phases,ggplot2::aes(x=cell,y=gene,fill=log_expression)) +
  ggplot2::geom_tile() + 
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank()) +
  ggplot2::scale_fill_gradient(low = "white",
                      high = "red",
                      guide = "colorbar",
                      limits=c(0, max(heatmap_counts))) + 
  ggplot2::ggtitle("Top Fourier Power Genes")


top_r2_spread_phases = heatmap_counts[rownames(heatmap_counts) %in% top_genes_fourier_top_r2_spread,]
top_r2_spread_phases = redim_matrix(top_r2_spread_phases, target_height = nrow(top_r2_spread_phases), target_width = 1000, n_core=2)
top_r2_spread_phases <- reshape2::melt(top_r2_spread_phases, c("gene", "cell"), value.name = "log_expression")
fourier_top_r2_spread_phases = ggplot2::ggplot(data=top_r2_spread_phases,ggplot2::aes(x=cell,y=gene,fill=log_expression)) +
  ggplot2::geom_tile() + 
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank()) +
  ggplot2::scale_fill_gradient(low = "white",
                      high = "red",
                      guide = "colorbar",
                      limits=c(0, max(heatmap_counts))) + 
  ggplot2::ggtitle("Top Fourier R2 Spread Genes")

top_amp_spread_phases = heatmap_counts[rownames(heatmap_counts) %in% top_genes_fourier_top_amp_spread,]
top_amp_spread_phases = redim_matrix(top_amp_spread_phases, target_height = nrow(top_amp_spread_phases), target_width = 1000, n_core=2)
top_amp_spread_phases <- reshape2::melt(top_amp_spread_phases, c("gene", "cell"), value.name = "log_expression")
fourier_top_amp_spread_phases = ggplot2::ggplot(data=top_amp_spread_phases,ggplot2::aes(x=cell,y=gene,fill=log_expression)) +
  ggplot2::geom_tile() + 
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank()) +
  ggplot2::scale_fill_gradient(low = "white",
                      high = "red",
                      guide = "colorbar",
                      limits=c(0, max(heatmap_counts))) + 
  ggplot2::ggtitle("Top Fourier Amp Spread Genes")

top_power_spread_phases = heatmap_counts[rownames(heatmap_counts) %in% top_genes_fourier_top_power_spread,]
top_power_spread_phases = redim_matrix(top_power_spread_phases, target_height = nrow(top_power_spread_phases), target_width = 1000, n_core=2)
top_power_spread_phases <- reshape2::melt(top_power_spread_phases, c("gene", "cell"), value.name = "log_expression")
fourier_top_power_spread_phases = ggplot2::ggplot(data=top_power_spread_phases,ggplot2::aes(x=cell,y=gene,fill=log_expression)) +
  ggplot2::geom_tile() + 
  ggplot2::theme(axis.text.x = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank()) +
  ggplot2::scale_fill_gradient(low = "white",
                      high = "red",
                      guide = "colorbar",
                      limits=c(0, max(heatmap_counts))) + 
  ggplot2::ggtitle("Top Fourier Power Spread Genes")
