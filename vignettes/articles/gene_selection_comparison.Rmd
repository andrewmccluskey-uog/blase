...
TODO: Switch to BLASE methods where possible.
...
We will investigate in detail the Bozdech method for gene selection described in PAPER.
...
x = as.AtgnatData(processed_MCA_PB_SCE, pseudotime_slot="slingPseudotime_1", n_bins=14, split_by="pseudotime_range")
...
x = as.AtgnatData(processed_MCA_PB_SCE, pseudotime_slot="slingPseudotime_1", n_bins=BUCKETS_COUNT, split_by="pseudotime_range")
...
waves$phase = waves$phase * (((180/3.141593)/360)*max(processed_MCA_PB_SCE$slingPseudotime_1)) # pseudotime
waves$gene = rownames(waves)
...
Here we've plotted the genes and cells ordered by their peak phase and pseudotime respectively. This functionality is available as a part of BLASE, please see the `gene_selection_matrix()` function.
```{r}
...
heatmap_counts=logcounts(processed_MCA_PB_SCE)[,order(processed_MCA_PB_SCE$slingPseudotime_1)]
small_heatmap_counts_ordered = redim_matrix(heatmap_counts[order(waves$phase),], target_height = 1000, target_width = 1000, n_core=N_CORES)
...
```
### Util
# TODO JUST USE THE FIXED FUNCTION FROM BLASE
```{r}
...
```{r}
waves$total_expression = rowSums(counts[,order(processed_MCA_PB_SCE$slingPseudotime_1)])
five_percent_of_pdt = 0.05*max(processed_MCA_PB_SCE$slingPseudotime_1) # Bozdech et al. use plus or minus 1/48 (i.e. one bulk either side of the peak)

# Review this works with someone because I think it might be questionable
waves$peak_expression = 0
for (gene in rownames(counts)) {
  waves[gene,"peak_expression"] = sum(counts[gene,processed_MCA_PB_SCE$slingPseudotime_1 > waves[gene,]$phase-five_percent_of_pdt & processed_MCA_PB_SCE$slingPseudotime_1 < waves[gene,]$phase+five_percent_of_pdt])
  waves[gene,"cellcount_in_peak"] = length(counts[gene,processed_MCA_PB_SCE$slingPseudotime_1 > waves[gene,]$phase-five_percent_of_pdt & processed_MCA_PB_SCE$slingPseudotime_1 < waves[gene,]$phase+five_percent_of_pdt])
}
...
fourier_phase_aligned_plot = function(heatmap_counts, genes_to_plot, title) {
  ...
  phases = redim_matrix(phases, target_height = nrow(phases), target_width = 1000, n_core=N_CORES)
...
Disabled as not sure we want to keep this section
```{r, eval=FALSE}
...
get_earthmovers_distance_from_diagonal = function (expression_matrix, title) {
  ...
  reduced_mat = redim_matrix(expression_matrix, target_height = nrow(expression_matrix), target_width = nrow(expression_matrix), n_core=N_CORES)
  reduced_sim_mat = redim_matrix(sim_mat, target_height = nrow(sim_mat), target_width = nrow(expression_matrix), n_core=N_CORES)
...
```{r}
heatmap_counts=logcounts(processed_MCA_PB_SCE)[order(waves$phase),order(processed_MCA_PB_SCE$slingPseudotime_1)]
