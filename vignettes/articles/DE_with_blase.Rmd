...
as.matrix(cbind(sce@colData["UMAP_2"], sce@colData["UMAP_3"]))
sce <- slingshot(sce, reducedDim = 'UMAP', clusterLabels="STAGE_LR", start.clus="ring")
```

```{r getDataIsThisBrokenStageLRPCA}
#plotPCA(sce, colour_by="STAGE_LR")
```
```{r getDataIsThisBrokenStageLRUMAP}
#plotUMAP(sce, colour_by="STAGE_LR")
```
```{r getDataIsThisBrokenPCA}
plotPCA(sce)
```
```{r getDataIsThisBrokenUMAP}
plotUMAP(sce)
```
```{r getDataIsThisBrokenDayPCA}
plotPCA(sce, colour_by="DAY")
```
```{r getDataIsThisBrokenDayUMAP}
plotUMAP(sce)
```
```{r getDataIsThisBrokenStrainPCA}
plotPCA(sce, colour_by="STRAIN")
```
```{r getDataIsThisBrokenStrainUMAP}
plotUMAP(sce, colour_by="STRAIN")
```
```{r getDataIsThisBrokenPseudotimePCA}
plotPCA(sce, colour_by="slingPseudotime_1")
```
```{r getDataIsThisBrokenPseudotimeUMAP}
plotUMAP(sce, colour_by="slingPseudotime_1")
```
```{r getDataIsThisBroken1}
