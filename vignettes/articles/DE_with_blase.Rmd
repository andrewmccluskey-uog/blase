...
rm (new_rows, rownames_to_remove, bulk_matrix, bulk_n, genes_to_fix_bulk)
gc()
```
...
rm (new_rows, rownames_to_remove, sc_readcounts_matrix, sc_readcounts_n, genes_to_fix)
gc()
```
...
```{r tradeSeq}
sce <- fitGAM(sce, parallel=T, BPPARAM=MulticoreParam(2), nknots=7)
