...
```{r}
sce <- fitGAM(sce, parallel=T, BPPARAM=MulticoreParam(3), nknots=7)
associationTestResult <- associationTest(sce, lineages=T, global=F, contrastType="consecutive")
```

## Compare Gene Lists
```{r}
gene_selection_matrix(sce)
```

## Get Fourier Genes
```{r}
waves = get_waves(sce, "slingPseudotime_1")
...
gene_selection_matrix(sce, genes=rownames(fourier_power_genes), target_matrix_size=50)
```
